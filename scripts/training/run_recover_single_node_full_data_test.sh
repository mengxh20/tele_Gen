...
export RECOVER_OUTPUT_DIR=/data/heli/code/tele_Gen/reconstruct/experiments/recover_sparse_refresh_full_data_single_node_lazy_test

export FULL_DATA_TEST_MAX_STEPS="${FULL_DATA_TEST_MAX_STEPS:-20}"
export FULL_DATA_TEST_EPOCHS="${FULL_DATA_TEST_EPOCHS:-1}"
export CHECKPOINT_EVERY_STEPS="${CHECKPOINT_EVERY_STEPS:-1000}"

echo "[INFO] role=single-node-full-data-test addr=${MASTER_ADDR}:${MASTER_PORT} nnodes=${NNODES} rank=${NODE_RANK}"
...
echo "[INFO] epochs=${FULL_DATA_TEST_EPOCHS} max_steps=${FULL_DATA_TEST_MAX_STEPS} checkpoint_every_steps=${CHECKPOINT_EVERY_STEPS} lazy_sequence_cache_size=1"
...
bash scripts/training/train_multinode.sh \
  ...
  --epochs "${FULL_DATA_TEST_EPOCHS}" \
  --max_steps "${FULL_DATA_TEST_MAX_STEPS}" \
